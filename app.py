...
def get_pretty_name(user_id):
    return slacker_client.users.get(user_id)['id']
...
def handle_command(parsed, score):
    ...
    if 'enroll' in parsed['type']:
        ...
        response = "Congratulations: user " + get_pretty_name(parsed['user']) + " is enrolled!"
    if 'kill' in parsed['type']:
        handle_kill(score, parsed['image'], get_pretty_name(parsed['user']), parsed['channel'])
        response = "User: " + get_pretty_name(parsed['user']) + " attemptd to kill someone!"
